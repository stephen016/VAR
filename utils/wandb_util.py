...
def initialize(args, entity, exp_name, project_name):
    ...
    wandb.login(key="05dc3ea957d086789d3b206bdd4196ba166e70ce")
    wandb.init(
        entity=entity,
...
def log_image_2(name, sample, step=None):
    if is_main_process():
        # sample = array2grid(sample)
        wandb.log({f"{name}": wandb.Image(sample), "train_step": step})

def log_dict(wandb_dict, commit):
    if is_main_process():
        wandb.log(wandb_dict, commit=commit)
